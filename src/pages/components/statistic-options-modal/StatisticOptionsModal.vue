...
const periodBegin = ref<Date>(dayjs().tz().startOf("week").toDate());
...
const initPeriodDates = (period: StatisticsPeriodType) => {
  ...
  switch (period) {
    case "Today":
      periodBegin.value = dayjs().startOf("day").toDate();
      break;
    case "Current Week":
      periodBegin.value = dayjs().startOf("week").toDate();
      break;
    case "Current Month":
      periodBegin.value = dayjs().startOf("month").toDate();
      break;
    case "Salary Month":
  ...
  periodEnd.value = dayjs().tz().startOf("minute").toDate();
};
...
const initPreOptionsByPeriod = () => {
  ...
  radioMoneyType.value = optionsObj.value.moneyType
    ? optionsObj.value.moneyType
    : "All";
...
const onDeleteTagFromCurrentOption = (tag: ITag) => {
  tagsForCurrentOptions.value = tagsForCurrentOptions.value.filter(
    (item) => item.Id !== tag.Id,
  );
};

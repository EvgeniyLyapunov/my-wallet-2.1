<template>
      ...
      <div class="card__main-info">
        ...
        <div class="card__main-info-block">
          ...
          <span class="card__main-info-value"
            ...
            <span
              v-if="card!.isVirtual"
              class="card__main-info-value_virtual"
              >{{ card!.baseCardName }}</span
            >
          </span>
        </div>
      </div>
  ...
  <CardInfo_EditModal
    v-model="isVisible_EditInfoModal"
    :card="card"
    @onEdit="onEditCardInfo"
  />
  <MessageBox
    v-model="isVisibleMessageBox"
...
onMounted(() => {
  cardHeight.value =
    elementHeight_Relative_PreviousSiblingAndWindowHeight(".card__title-block");
});
...
const onEditCardModalOpen = () => {
  if (card.value!.virtualList.length > 0) {
    ...
    messageBoxMessage.value =
      "Эта карта имеет виртуальные карты. Редактирование невозможно!";
  } else {
